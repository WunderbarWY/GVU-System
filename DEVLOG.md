...
### 新增的
- **Canvas 战术渲染层**：`tactical-renderer.js` 接管轨道、航道、舰船、威胁圈、引擎尾焰、选中特效的绘制
  - DOM 单位保留为无障碍/命中热区，高频绘制交给 Canvas
  - 世界尺寸从 7200×4700 放大到 16000×10400，提升战略纵深感
  - 新增 Canvas 特效：`lock` / `warp` / `strike`

### 改动的
- **缩放体验重构**：从"固定中心加减缩放"改为"以指针/舞台中心为焦点的指数缩放"
  - 滚轮缩放：以鼠标位置为焦点，每步约 12%，支持 trackpad 加速度
  - 按钮缩放：以舞台中心为焦点，放大 1.25x / 缩小 0.8x
- **安全加固**：`escapeHtml` 全面覆盖任务详情、战史、舰队卡片、威胁圈/尾迹等外部数据注入点
- **测试修复**：`core.test.js` 中 `cssEscape` 实现同步为完整版，39 个测试全部通过
- **缓存策略**：统一所有 CSS/JS 资源 cache-bust 为 `?v=zoomfix-1`
